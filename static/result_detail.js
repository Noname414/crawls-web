...
function processResultData() {
    ...
    document.getElementById('resultTitle').innerHTML = 
        `<i class="fas fa-search me-2"></i>「${keyword}」搜尋結果`;
}
...
function renderProducts() {
    ...
    document.getElementById('filterInfo').textContent = 
        `顯示 ${startIndex + 1}-${Math.min(endIndex, filteredProducts.length)} 個商品，共 ${filteredProducts.length} 個`;
    ...
    row.innerHTML = `
        <td>
            <img src="${product.image_url || '/static/placeholder.jpg'}" 
                 ...
                 alt="${product.title}" onerror="this.src='/static/placeholder.jpg'">
        </td>
        <td>
            <div class="product-title">${product.title}</div>
        </td>
        <td>
            <strong class="text-primary">NT$ ${(product.price || 0).toLocaleString()}</strong>
        </td>
        <td>
